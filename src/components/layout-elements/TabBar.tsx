...
export function TabBar({ className }: ClassNameProps) {
  ...
  return (
    <div
      className={twMerge(
        // Home is lifted out of the row into a raised disc that overhangs the
        // top edge, so the bar must not clip its own children.
    ...
    >
      ...
      {tabs.map(({ route, icon, name }, index) => {
        ...
        return route === routes.home ? (
          <TabBarCenterItem key={route} {...shared} />
        ) : (
          <TabBarItem
            key={route}
            {...shared}
