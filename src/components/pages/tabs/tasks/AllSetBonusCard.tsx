 ...
 *
 * It gets its own card above the list instead of a row inside a category,
 * because it is the only task whose condition is *the other tasks*. As a
 * Tournaments row it read as one more tier task, and with Bronze the only
 * auto-spawned tier its condition was literally `task-daily-bronze`'s — the
 * same four entries, paid a second time and six times over, under a Gold medal
 * it had no claim to. Here the checklist says outright what is left to do.
 *
 * The sub-steps are read-only: each one is another task with its own card and
 * its own reward, so there is nothing to collect on this one until the whole
 * set is done. The backend sends them with `claimable: false` and never accepts
 * a sub-step claim for this task.
 */
export function AllSetBonusCard({
  ...
  return (
    <section className={twMerge('animate-slide-in-bottom px-4 pb-3', className)} style={style}>
      <div
        className={twMerge(
