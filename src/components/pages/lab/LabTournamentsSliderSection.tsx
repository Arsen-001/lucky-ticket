...
export function LabTournamentsSliderSection() {
  ...
  return (
    <LabSection
      ...
      bleed
      note="Было: одна карточка 256×64 в кадре, автопрокрутка каждые 2 секунды, соседи срезаны маской, порядок случайный, заголовка нет — чтобы увидеть все семь турниров, надо было простоять у экрана 14 секунд. Выбран вариант C, он стоит на главной. Во всех вариантах: ближайший старт первым, автопрокрутки нет, над лентой заголовок со счётчиком и выходом в каталог."
    >
      <LabVariant
        bleed
        label="C · билет · ПРИНЯТ, стоит на главной"
        bet="Строка, вырезанная билетом: слева оторванный корешок с медалью, тир заливает его целиком, пунктир отрыва и два прокола, сквозь которые виден фон. В кадре полтора билета, и половинка читаемая — у соседа видно тир и название. Блок 120px против 84px у прежней карусели: главная почти не выросла. Это настоящий компонент главной, а не копия."
      >
      ...
      <LabVariant
        bleed
        label="A+C · афиша-билет · стоял на главной час"
        bet="Гибрид: пропорции A (вертикальная карточка, приз крупнейшим кеглем, медаль 60px) и та же форма билета, только шов идёт поперёк — отсчёт стоит на оторванном корешке внизу. Витрина богаче, но блок 188px против 120px: за это платит двигатель, который уезжает ниже."
      >
        <LabTournamentPosterTicketStrip tournaments={items} />
      </LabVariant>
