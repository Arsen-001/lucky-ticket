...
const VARIANTS: { key: LabTabBarVariant; label: string; bet: string }[] = [
  ...
  {
    key: 'center',
    label: 'Т3 · главная выносной кнопкой',
    bet: 'Центр выходит из ряда — «Главная» становится якорем и попадает под большой палец. Игровой вид, но кнопка съедает 28px над панелью и ломает ровный ряд.',
  },
  {
    key: 'stacked',
...
const SIGNATURE: { key: LabTabBarSignatureVariant; label: string; bet: string }[] = [
  {
    key: 'ticket',
    label: 'С1 · панель — это билет · ПРИНЯТ',
    bet: 'Выбран и перенесён в приложение — настоящая панель внизу экрана теперь такая же. Верхний край прорезан полукруглой перфорацией, между вкладками — линия отрыва, активная вкладка выглядит как оторванный корешок: подсвечена от линии отрыва вниз и приподнята.',
  },
  {
    key: 'ball',
  ...
  return (
    <>
      ...
      <LabSection
        ...
        note="Четыре привычных решения, оставлены для сравнения. Прежней панели-пилюли среди них нет: её место занял билет (С1), он же теперь внизу экрана."
      >
        {VARIANTS.map(v => (
          <LabVariant key={v.key} label={v.label} bet={v.bet}>
            <div className="-mx-4 overflow-hidden">
