...
const VARIANTS: { key: LabTabBarVariant; label: string; bet: string }[] = [
  ...
  {
    key: 'center',
    label: 'Т3 · главная выносной кнопкой · ПРИНЯТА КНОПКА',
    bet: 'В приложение взята сама выносная кнопка — она стоит на билетной ленте С1, а не на этой обычной панели. Центр выходит из ряда: «Главная» становится якорем и попадает под большой палец. Игровой вид, но кнопка съедает 14px над панелью и ломает ровный ряд.',
  },
  {
    key: 'stacked',
...
const SIGNATURE: { key: LabTabBarSignatureVariant; label: string; bet: string }[] = [
  {
    ...
    bet: 'Выбран и перенесён в приложение — настоящая панель внизу экрана такая же, только «Главная» вынесена из ряда в приподнятый диск (кнопка из Т3). Верхний край прорезан полукруглой перфорацией, между вкладками — линия отрыва, активная вкладка выглядит как оторванный корешок: подсвечена от линии отрыва вниз и приподнята.',
  },
  {
    key: 'ball',
  ...
  return (
    <>
      ...
      <LabSection
        ...
        note="Четыре привычных решения, оставлены для сравнения. Прежней панели-пилюли среди них нет: её место занял билет (С1). Из этой четвёрки в приложение уехала только выносная кнопка Т3 — она стоит на билете."
      >
        {VARIANTS.map(v => (
          <LabVariant key={v.key} label={v.label} bet={v.bet}>
            <div className="-mx-4 overflow-hidden">
