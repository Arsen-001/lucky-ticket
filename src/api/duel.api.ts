...
export const duelApi = api.injectEndpoints({
  endpoints: builder => ({
     ...
     *
     * Ответ разделяет три факта: `invited` — скольким вызов дошёл в игру (это
     * работает всегда), `sent` — скольким ушло ещё и письмо (бот не может
     * написать первым, так что это меньшее число), `skipped` — сколько
     * отказано их собственной настройкой «кто может меня звать».
     */
    inviteToDuel: builder.mutation<
      { invited: number; sent: number; skipped: number },
