...
**The status screens quote the live config, never a written list.** `/settings/lucky-player` and `/settings/vip` build their privilege rows from the perks the backend ships with the catalog listing (`market.statuses[].perks` for LP, `levelPerks` for the VIP ladder, plus `perkBase` for the free limits a counted perk is added to). **A perk the admin sets to 0 produces no row at all** — the screens may not advertise what the server will not grant. VIP additionally shows two lists: the level the player holds, and only what the next level changes. Until 2026‑08‑09 both screens rendered a static list of i18n keys seeded into the catalog at first boot; it had drifted from the config on nearly every row (it promised +25% engine speed against a live 20%, a stake-fee discount that is 0 on all twenty levels, and tournament perks nobody grants). The same rule now applies to the FAQ entries for both statuses: they describe the perk **categories** and point at the screen, and carry no numbers.
...
**The daily set a brand-new account sees.** Four cards plus the ads block: join the day's Bronze brackets (`task-daily-bronze`), claim what the starter engine produced (`t-246`, counter `daily_tickets`), watch the day's ten rewarded ads (`task-daily-ads`, counter `daily_ads`, target = the free cap), the channel check-in (`t-258`), and the all-set bonus over the other three (§12.4). Every one of them is driven by a live counter — produce → play → watch → subscribe. `t-246` used to be "Share your daily result", which had no share mechanic anywhere in the app, no external link and a deeplink pointing at the tasks screen it already sat on: an always-claimable button worth 1 AP a day for a tap. It was repurposed rather than deleted, because the day needs an action a zero-balance account can always take, and its engine is already running.
...
**Binding a wallet is open to everyone (2026-08-11).** The way **in** — connect a wallet, deposit TON, buy Stars — carries no switch and no invite gate on production: `connectEnabled` is **on** and `connectMinReferrals` is **0**. Only the way **out** is closed (`withdrawalsEnabled`, §15). The rule this replaces is worth stating because it lasted a week: from 2026-08-05 binding was shut alongside withdrawals, which also shut TON deposits, since a deposit needs a bound wallet. That cost the platform its only inbound money path in order to close an exit that was already closed on its own switch.

**The switch itself stays as a kill switch: `walletConfig.connectEnabled`.** While it is off, `POST /wallet/connect` answers `403 { error: 'wallet-connect-disabled' }`, the wallet state carries `connectEnabled: false`, and the Mini App draws "wallet connection is temporarily closed" (`TonWalletClosed`) in place of the hero card — with the deposit / withdraw / exchange row removed, since all three are TON-only and would be dead buttons. Stars, LC and every in-app spend keep working; the switch closes the TON wallet, not the account. It is separate from `withdrawalsEnabled` (§15) on purpose: that one shuts the way out while the wallet stays usable, this one shuts the door itself. **The invite gate below must never be shown in its place** — a progress bar reading `0/1` promises an unlock that inviting a friend cannot deliver, and that mismatch is exactly why the switch exists: during the closed test, players with one invited friend were connecting wallets on a screen that announced the feature as unavailable. Accounts that already bound an address are grandfathered past it (same rule as the invite gate below): their TON is real, and a dropped TON Connect session must not strand it behind a switch nothing they do can flip.
...
| ----------------------------------- | ----------------------- | ----------------------------- | --------------------------------------------------- |
| `walletConfig.connectMinReferrals`  | `POST /wallet/connect`  | **0 — no gate on production** | **Yes** — an address bound earlier stays bindable.  |
...
**Both converters take input on either side (2026-08-11).** In the TON→LS exchange and in the LC→TON conversion, the player may type into the FROM box or the TO box: "how many stars for 2 TON" and "how much TON for 500 stars" are the same quote read in either direction, and until now only the first had an answer — the second was found by trying amounts against a rate the screen never showed. The inverse of each quote is a named util next to the forward one (`tonToLc` beside `lcToTon`, `starsToTon` beside `tonToStars`) and `tests/converter-two-way.test.ts` pins the pair so they cannot drift. Whichever box was typed into stays the authority and the other is derived from it on every render — never both stored, which is how a converter walks 5 → 4.999 → 4.998 across keystrokes. `tonToLc` rounds **up**: LC is a whole unit, so the quote must buy at least the TON that was asked for. The stars leg is not a clean round trip (the charge rounds up to the milli-TON, then floors back into whole stars) and it does not need to be: the typed count is what the purchase sends, and the TON box shows the exact charge.
...
**Money comes in freely and does not leave — with no date named.** Since 2026-08-05 `withdrawalsEnabled` is **off** on production: both exits — `POST /wallet/withdraw` and LC→TON conversion — are closed and are switched back on **by hand**. The Telegram-Stars purchase, the TON→LS exchange and every in-app spend are unaffected, and since 2026-08-11 so is the whole way **in**: binding a wallet and depositing TON are open to everyone (§15.1). Between 2026-08-05 and 2026-08-11 they were shut too, by `connectEnabled` — that closed the platform's only inbound money path in order to guard an exit its own switch already guarded.

**The copy names no date.** The lock used to read "opens after the test period"; it now reads "temporarily closed", because the reopening is a manual toggle rather than a rule keyed on `testQuestConfig.testEndsAt` — the same shape as `leaderboardEnabled` (§13). A date in the copy is a promise the toggle does not make: the date can move, and a money exit that appears to open itself on a stale date is the wrong failure. The reason for closing it is that test-period balances are not launch-period balances — LS faucets pay out during the test and tournament prize pools are split between a handful of players (§2.8) — so a withdrawal now would price the test's inflation at mainnet rates.

**The closed exit is a rendered state, not an error.** `GET /config` publishes `wallet.withdrawalsEnabled`, and both exit screens — the TON withdrawal modal and the LC→TON conversion — draw a "temporarily closed" lock in place of their form (`WalletWithdrawLocked`). That lock outranks the invite gates above it: while nothing can leave at all, how many friends were invited is beside the point. On the wallet screen the **Withdraw** button itself carries a padlock and drains to grey, because the two buttons beside it work: an unmarked button that answers "closed" reads as this screen being broken, not as a policy. It is marked, not disabled — the tap still opens the modal that explains, and a dead button explains nothing. Tapping it never starts a TON Connect sheet either, even with no wallet bound: binding does not open this door. Both endpoints answer `403 { error: 'withdrawals-disabled' }`, and the client reads that at submit time too, so an exit closed between the config query and the tap turns into the same lock rather than a toast. A backend that doesn't publish the field is treated as open — the 403 is still the last word, and hiding a working feature because a flag is missing would be the worse default. Until 2026-08-05 the switch reached the player only as that 403, after an address and an amount had been typed, and the modal reported it as a generic "action failed": a closed door rendered as a broken button.
...
- **Tournament LC reward:** whenever a referred friend takes a prize in a tournament, **4%** of that prize accrues to the inviter as claimable LC (`referralConfig.tournamentLcPct`, admin-tunable, `0` disables it). The number is **flat — the friend's status does not change it.** It replaced a four-step ladder keyed to the friend's account (5 / 10 / 15 / 25% for regular / Premium / Lucky Player / VIP), which was both impossible to state in one sentence on the screen and a perk that paid the wrong person: being VIP raised what somebody _else_ collected off you. The VIP/LP "referral %" perk line is gone from both status perk lists with it.
- **Second level: 1% from a friend's friends.** A prize taken by someone a _referred friend_ invited pays **1%** to the level above as well (`referralConfig.tournamentLcL2Pct`, admin-tunable, `0` switches the second level off without touching the first). It stops there — there is no third level.
  - **A split of the old 5%, not an addition.** The pair `4 + 1` is exactly what the single flat level minted before it, so total emission per prize does not rise — and falls slightly in practice, because only some players have a grandparent at all (69% of referral edges on prod, measured 2026-08-10). Break-even for an inviter is the same under either split (`L1 + L2×B = 5` ⇒ a friend must bring `B ≥ 1`); `4/1` was chosen over `3.5/1.5` because the extra half-point buys no additional incentive at that threshold and only moves money from the ~90% of inviters whose friends bring nobody to the ~10% who build deep networks.
  - **Pooled per branch, never per grandchild.** The accrual is keyed to the **middle link**, not to the winner: one `ReferralLcClaimable` row (`level = 2`) per (inviter, direct friend) holds everything that friend's whole branch has produced. So the second level needs no second claim button, no second gate and no grandchild identity — the friend's own claim pays their branch, and the friend's own «не засчитан» verdict freezes it. A branch hanging off a friend who left the channel is frozen, not burnt, exactly like their own reward.
  - **Cycles cannot pay twice.** `A→B` and `B→A` can both exist (each row is unique only in its referred user), so walking one level up can land back on the winner. The guard that refuses a self-payment at the first level is applied again at the second.
  - **The player sees the count, never the people.** A direct friend arrived through _your_ link, so showing their profile needs no further consent; the players in their branch have no relationship with you at all. The friends screen therefore shows a **«привёл N» badge per friend** and one **«их друзья»** line in the hero (LC first, headcount second) — and never a name, avatar or profile from the second level. The badges sum to the hero figure by construction: both are counted off `Referral` rows rather than the denormalized `User.referralsCount`, which a deleted account decrements in one place and not the other.
  - **Nothing else moves.** `referralsCount` — and with it tier requirements (§5.2), the wallet gates (§16.4) and the Friends milestones (§21) — counts **direct invites only**. Second-level people are not referrals of the level above; folding them in would inflate tiers by a factor of the branching.
- **Minted on top.** The friend's own prize is untouched — a 10 000 LC win pays the friend 10 000, mints 400 for the inviter and 100 for the level above. Rounding is always **down**, per level, so a prize too small to pay a whole coin pays nothing rather than inventing one (which bites the second level four times sooner than the first).
...
- **Claim Mechanic:** the LC is not credited instantly — it accumulates per friend and is claimed from the friends screen, individually or with «Забрать всё». One press pays that friend's own cut **and** their branch; the two are written as **separate ledger entries** (`Referral reward: friend tournament winnings` / `… friend network tournament winnings`) so the economy report can price the second level apart from the first. The claim sheet is the one place the split is shown to the player («свои X · сеть Y»); the row and the header state one figure, because one button pays it.
