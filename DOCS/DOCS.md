...
- **What each level gives is now defined by explicit lookup tables** (`ticket-engine.utils.ts`), not a fixed per-level scalar — edit a table cell to reshape any curve (it need not be linear). The four tables and their **default** values:
  - Each **engine level** contributes a per-level speed **%** (`ENGINE_LEVEL_SPEED_BOOST_PCT_TABLE`, default `0 / 100 / 200 / 300 / 400` → `+100%` per level above 1) **and** an **absolute base per-cycle output** (`ENGINE_LEVEL_BASE_CAPACITY_TABLE`, default `1 → 11 → 21 → 31 → 41`). Engine level is reached by **promotion** — see Section 10.2.
  ...
  - Each **capacity-level upgrade** contributes `CAPACITY_LEVEL_BOOST_PCT_TABLE[level]` (default `+10%`/level, `0…100%`) — so at the default curve a fully capacity-upgraded engine mints **2 tickets per cycle** _at engine level 1_ (base 1 → 2). At higher levels the same total % applies to the bigger base, e.g. a level-2 engine's maxed capacity mints `round(11 × 2) = 22`.
...
- sets **base per-cycle output** to the table value (`ENGINE_LEVEL_BASE_CAPACITY_TABLE`, default `1 → 11 → 21 → 31 → 41`).
...
**Engine-level cap.** The engine level itself is capped at **5** (`MAX_ENGINE_LEVEL`, derived from the level-table length; the backend mirrors the same tables in `economy.constants.ts` and enforces the cap server-side in its upgrade path). At the cap the 10/10 sub-level state is **terminal** — the ladders stay full and no further promotion occurs. `tests/engine-table-parity.test.ts` diffs the four tables between the two repos, so a one-sided balance edit fails CI instead of silently drifting.
