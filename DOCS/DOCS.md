...
**Item photos:** the icon stage renders the item's `imageUrl` when the catalog provides one, otherwise a category/tier icon. The **tier-bound SKUs (engine / ticket / shard)** ship with a **backend-served default photo** — the real per-tier art in the Mini App's `public/assets/icons/…` — set at seed time and re-backfilled on every boot (`MarketService.syncCatalogImages`) for catalogs seeded before the photos existed. The backfill only fills a NULL `imageUrl`, so an admin-uploaded custom photo (Admin → Маркет) always wins. Paths are Mini-App-relative, so the admin panel (separate origin) shows the category icon for them rather than a broken image.
