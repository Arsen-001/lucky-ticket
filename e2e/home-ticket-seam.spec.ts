...
/**
 * Where the punches are, measured from the card's left edge.
 *
 * The stub hangs off the READING-START edge, so in Arabic and Persian it is on
 * the right — but a gradient position is physical and does not flip on its own.
 * It did not: the card came out torn down the side opposite its own medal, with
 * the dashed seam cutting through the countdown, and this test never saw it
 * because it only ever ran in English.
 */
const punchX = (card: Card) => (card.rtl ? card.width - card.stub - 2 : card.stub + 2);

/** English and one right-to-left language — the ticket is torn in both. */
for (const locale of ['en', 'ar'] as const) {
  test.describe(`home ticket seam in ${locale}`, () => {
    test('both punches are cut out of the card', async ({ page }) => {
      ...
      for (let attempt = 0; attempt < 15 && !painted; attempt++) {
        ...
        const x = punchX(before); // clear of the seam's own dashed border
        const points = {
          topPunch: [x, 2],
